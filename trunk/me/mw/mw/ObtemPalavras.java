...
public class ObtemPalavras extends MIDlet implements CommandListener, Runnable {
	...
	public ObtemPalavras() {
		...
		mProgressString.setText("Obtém mais de 2000 palavras e as persiste no arquivo 'palavras.txt'");
	...
	public Vector getWords(String base, String file) {
		...
		try {
			...
			while ((indice = data.indexOf("<li>", indice + 3)) != -1) {
				...
				list.addElement(data.substring(start, end).trim());
			}
		} catch (Exception e) {
			return null;
		}
	...
	public Vector getMostFrequently3000UsedWords() {
		...
		for (int i = 1; i < 30; i += 2) {
			...
			mProgressString.setText("Obtido " + i + " de 30");
		}
		return allWords;
	}
	
	public boolean saveStringVectorInFile(Vector vector, String fileName) {
		OutputStream os = Utils.getOutputStream(fileName);
		OutputStreamWriter osw = new OutputStreamWriter(os);
		if (osw == null)
			return false;
		
		Enumeration vetor = vector.elements();
		while (vetor.hasMoreElements()) {
			try {
				String palavra = (String) vetor.nextElement();
				osw.write(palavra + "\n");
				mProgressString.setText(palavra);
			} catch (IOException e) {
				e.printStackTrace();
				return false;
			}
	...
	public void run() {
		...
		if (saveStringVectorInFile(allWords, "////Storage Card/palavras.txt")) {
...
//		// Get the corresponding wave file´s URL
//
...
//		}
		mProgressForm.removeCommand(mInterromperCommand);
		mProgressForm.addCommand(mExitCommand);
	}
	...
	/**
	 * Dada uma determinada URL, este mÃ©todo lÃª linha por linha desta URL e
	 * repassa a linha correspondente para instÃ¢ncia de ProcessaLinha.
	 * 
	 * @param pl
	 * @param url
	 */
