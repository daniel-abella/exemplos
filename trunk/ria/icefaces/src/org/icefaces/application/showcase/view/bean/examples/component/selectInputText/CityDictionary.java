 ...
 * SelectInputTextBean as the cityDictionary is rather large.
 * </p>
 * <p>
 * This class loads the city data from an xml data set. Once an instance is
 * created a call to #generateCityMatches will generate a list of potential
 * matches.
 * </p>
 * 
 * @since 1.7
...
public class CityDictionary implements Serializable {
	...
	public CityDictionary() {
		...
		try {
			log
					.info(MessageBundleLoader
							.getMessage("bean.selectInputText.info.initializingDictionary"));
			...
			if (log.isErrorEnabled()) {
				log
						.error(MessageBundleLoader
								.getMessage("bean.selectInputText.error.initializingDictionary"));
			}
		}
	}
	...
	public ArrayList generateCityMatches(String searchWord, int maxMatches) {
		...
		try {
			...
			int insert = Collections.binarySearch(cityDictionary,
					new SelectItem("", searchWord), LABEL_COMPARATOR);
			...
			log
					.error(
							MessageBundleLoader
									.getMessage("bean.selectInputText.error.findingMatches"),
							e);
		}
	...
	private static void init() throws IOException {
		...
		if (!initialized) {
			...
			// Linha acima (comentada) foi substituída por aquela abaixo[xxx]
			...
			for (int i = 0, max = cityList.size(); i < max; i++) {
				...
				if (tmpCity != null && tmpCity.getCity() != null) {
					cityDictionary.add(new SelectItem(tmpCity, tmpCity
							.getCity()));
				}
			}
			cityList.clear();
