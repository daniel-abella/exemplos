...
/**
 * <p>
 * The SelectionTagsBean Class is the backing bean for the selection components
 * demonstration. It is used to store the options and selected values of the
 * various selection components.
 * </p>
 * <p>
 * For Developers new to JSF, the SelectItem object is a key class for working
 * with ICEfaces standard enhanced components. It can be added as the
 * &lt;f:selectItems /&gt; tags value attribute to generate list data. JSF takes
 * care of the default value and value binding in general.
 * </p>
 * <p>
 * Two gotcha when using and of the selection tags with JSF 1.1.
 * <li>use boolean primative instead of the Boolean Object</li>
 * <li>selection tags require String object. If you want to use another object
 * type for list use a Converter.</li>
 * </p>
 * 
 * @sinse 0.3.0
 */
public class SelectionTagsBean extends BaseBean {
	...
	private static final SelectItem[] DRINK_ITEMS = new SelectItem[] {
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.drink.drink1.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.drink.drink2.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.drink.drink3.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.drink.drink4.value")), };
	private static final SelectItem[] LANGUAGE_ITEMS = new SelectItem[] {
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.language.lang1.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.language.lang2.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.language.lang3.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.language.lang4.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.language.lang5.value")), };
	private static final SelectItem[] COMPONENT_ITEMS = new SelectItem[] {
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.component.comp1.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.component.comp2.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.component.comp3.value")), };
	private static final SelectItem[] CARS_ITEMS = new SelectItem[] {
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.cars.car1.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.cars.car2.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.cars.car3.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.cars.car4.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.cars.car5.value")), };
	...
	private static final String COUNTRY_BRASIL = MessageBundleLoader
			.getMessage("bean.selection.country0.value");
	private static final String COUNTRY_CANADA = MessageBundleLoader
			.getMessage("bean.selection.country1.value");
	private static final String COUNTRY_USA = MessageBundleLoader
			.getMessage("bean.selection.country2.value");
	private static final String COUNTRY_CHINA = MessageBundleLoader
			.getMessage("bean.selection.country3.value");
	private static final String COUNTRY_UK = MessageBundleLoader
			.getMessage("bean.selection.country4.value");
	private static final String COUNTRY_RUSSIA = MessageBundleLoader
			.getMessage("bean.selection.country5.value");

	private static final SelectItem[] COUNTRY_ITEMS = new SelectItem[] {
			new SelectItem(COUNTRY_BRASIL), new SelectItem(COUNTRY_CANADA),
			new SelectItem(COUNTRY_USA), new SelectItem(COUNTRY_CHINA),
			new SelectItem(COUNTRY_UK), new SelectItem(COUNTRY_RUSSIA) };

	private static final SelectItem[] CITIES_BRASIL = new SelectItem[] {
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country0.city1.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country0.city2.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country0.city3.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country0.city4.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country0.city5.value")) };

	private static final SelectItem[] CITIES_CANADA = new SelectItem[] {
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country1.city1.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country1.city2.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country1.city3.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country1.city4.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country1.city5.value")) };

	private static final SelectItem[] CITIES_USA = new SelectItem[] {
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country2.city1.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country2.city2.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country2.city3.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country2.city4.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country2.city5.value")) };

	private static final SelectItem[] CITIES_CHINA = new SelectItem[] {
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country3.city1.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country3.city2.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country3.city3.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country3.city4.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country3.city5.value")) };

	private static final SelectItem[] CITIES_UK = new SelectItem[] {
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country4.city1.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country4.city2.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country4.city3.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country4.city4.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country4.city5.value")) };

	private static final SelectItem[] CITIES_RUSSIA = new SelectItem[] {
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country5.city1.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country5.city2.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country5.city3.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country5.city4.value")),
			new SelectItem(MessageBundleLoader
					.getMessage("bean.selection.country5.city5.value")) };
	...
	public void countryChanged(ValueChangeEvent event) {
		...
		if (COUNTRY_BRASIL.equals(newCountry)) {
			cityItems = CITIES_BRASIL;
		} else if (COUNTRY_CANADA.equals(newCountry)) {
	 ...
	 * cities. No actual work is done for this method but it does show what a
	 * ValueChange method signature should look like.
	 * 
	 * @param event
	...
	public String getSelectedCitiesStrings() {
		// if the changeEventListener fired then we want to clear the cities
		// list
		if (countryChange) {
			countryChange = false;
			return "";
